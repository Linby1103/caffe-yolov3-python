...
def rects_prepare(output, inp_dim=416, num_classes=80):
    ...
    for key, value in output.items():
        ...
        if prediction is None:
            ...
            prediction = predict_transform(value, anchors=anchors,num_classes=input_num_classes)
        else:
            prediction = np.concatenate([prediction, predict_transform(value, anchors=anchors,num_classes=input_num_classes)], axis=1)
    ...
    while ind < img_result.shape[0]:
        ...
        nms_mask = np.expand_dims(ious < nms_threshold, axis=-1)
